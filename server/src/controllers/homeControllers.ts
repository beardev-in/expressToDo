...
import { CustomRequest } from "express-serve-static-core";
...
*/
async function userRegisterController (req : CustomRequest, res : Response) {
    try {
...
async function userLoginController(req : CustomRequest, res : Response) {
    try {
        ...
        let userSessionDataRaw = await redisClient.get(user._id.toString()); 
        let userSessionData : UserSessionData = JSON.parse(String(userSessionDataRaw));
        ...
        if (userSessionData && userSessionData.activeSession == req.sessionID) {
            ...
            await redisClient.set(
                String(user._id),
                JSON.stringify({ activeSession: req.sessionID }),
                {
                  EX: 14400 // expiration time in seconds
                });
        } 
        const secretKey: Secret = process.env.JWT_SECRET_KEY as string;
...
*/
async function userLogoutController(req : CustomRequest, res : Response) {
    try {
...
*/
async function getLoggedInUserController(req : CustomRequest, res : Response){
    try{
...
*/
async function updateUserController(req : CustomRequest, res : Response) {
    try{
...
*/
async function uploadAvatarController(req : CustomRequest, res : Response){
    try{
        if (!req.file) {
...
Notes: API call to upload user avatar file
*/
async function getAvatarController(req : CustomRequest, res : Response) {
    try{
        let userId  = req.params.userId;
        let user = await UserModel.findById(userId);
        if(!user || !user.avatar) return res.status(400).json({error : {msg : "avatar not found!", path: "userStatus"}});
...
*/
async function deleteAvatarController(req : CustomRequest, res : Response){
    try{
...
*/
async function deleteUserController(req : CustomRequest, res : Response){
    try{
        let userId = req.userId;
